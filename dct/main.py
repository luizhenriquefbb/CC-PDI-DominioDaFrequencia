...
from selectPixels import selectImportantPixels
...
def main():
    ...
    important = selectImportantPixels(dtc2d_ret, n=args.pixelNumber)
    
    cv2.imwrite(args.output, dtc2d_ret)
    cv2.imwrite(args.output+"-important.jpg", important)

    cv2.imshow("Original", img)
    cv2.imshow("DCT Result", dtc2d_ret.astype(uint8))
    cv2.imshow("IDCT Result", idtc2d_ret.astype(uint8))
    cv2.imshow("important pixels", important.astype(uint8))
    cv2.waitKey(0)
